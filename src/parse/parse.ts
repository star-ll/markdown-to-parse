...
export function parseOrderedList(context: ParseContext, ancestors: MdAst[]) {
  ...
  arr.forEach((item, index) => {
    if (/^[0-9]+\.\s+/.test(item)) {
      ...
      orderedListItemAst.children.push({
        type: "Text",
        rowType: RowType.Block,
        children: [
          {
    ...
    } else if (/^(\s)+[0-9]+\.\s+/.test(item)) {
      ancestors.push(
        orderedListAst.children[orderedListAst.children.length - 1]
      );
      //   debugger;
      parseMarkdown(
        {
          ...context,
          source: item.replace(/^(\s)+/, ""),
        },
        ancestors
      );
      ancestors.pop();
